...
export type McuTransportButton =
  "rewind" | "fast-forward" | "stop" | "play" | "record";

export type McuGlobalLed = "smpte" | "beats" | "rude-solo";
export type XTouchColor =
  "black" | "red" | "green" | "yellow" | "blue" | "magenta" | "cyan" | "white";

export type McuInputMessage =
  | { type: "fader"; fader: number; position: number }
  | { type: "fader-touch"; fader: number; pressed: boolean }
  | { type: "encoder"; encoder: number; delta: number }
  | { type: "jog"; delta: number }
  | { type: "encoder-press"; encoder: number; pressed: boolean }
  | {
      type: "channel-button";
...
export function decodeMcuInput(
  data: ArrayLike<number>,
): McuInputMessage | null {
  ...
  if ((type !== 0x80 && type !== 0x90) || channel !== 0) return null;
  const pressed = type === 0x90 && second > 0;

  if (
    first >= MCU_FADER_TOUCH_NOTE_FIRST &&
  ...
  if (
    first >= MCU_ENCODER_PRESS_NOTE_FIRST &&
  ...
  if (first >= MCU_TRANSPORT_NOTE_FIRST && first <= MCU_TRANSPORT_NOTE_LAST) {
    const buttons: McuTransportButton[] = [
      "rewind",
      "fast-forward",
      "stop",
      "play",
  ...
  if (first === 0x2e) return { type: "bank-shift", delta: -8 };
  if (first === 0x2f) return { type: "bank-shift", delta: 8 };
  if (first === 0x30) return { type: "bank-shift", delta: -1 };
  if (first === 0x31) return { type: "bank-shift", delta: 1 };
  return null;
}

export function mcuFaderPositionToFraction(position: number): number {
  return Math.min(MCU_FADER_MAX, Math.max(0, position)) / MCU_FADER_MAX;
}

export function faderFractionToMcuFaderPosition(fraction: number): number {
  return Math.round(Math.min(1, Math.max(0, fraction)) * MCU_FADER_MAX);
}

export function mcuFaderMessage(
  fader: number,
  position: number,
...
export function mcuEncoderRingMessage(
  encoder: number,
  pan: number,
): [number, number, number] {
...
export function mcuEncoderRingOffMessage(
  encoder: number,
): [number, number, number] {
  const safeEncoder = Math.min(MCU_CHANNEL_COUNT - 1, Math.max(0, encoder));
...
export function mcuChannelButtonLedMessage(
  button: McuChannelButton,
  channel: number,
  active: boolean,
): [number, number, number] {
  const safeChannel = Math.min(MCU_CHANNEL_COUNT - 1, Math.max(0, channel));
  const firstNotes: Record<McuChannelButton, number> = {
...
export function mcuTransportLedMessage(
  button: McuTransportButton,
  active: boolean,
): [number, number, number] {
  const notes: Record<McuTransportButton, number> = {
    rewind: 0x5b,
    "fast-forward": 0x5c,
    stop: 0x5d,
    play: 0x5e,
...
export function mcuGlobalLedMessage(
  led: McuGlobalLed,
  active: boolean,
): [number, number, number] {
  const notes: Record<McuGlobalLed, number> = {
    smpte: 0x71,
    beats: 0x72,
    "rude-solo": 0x73,
  };
...
function mcuDisplayDigitMessage(
  cc: number,
  character: string,
  dot: boolean,
): [number, number, number] {
...
export function mcuTimeDisplayDigitMessage(
  position: number,
  character: string,
  dot = false,
): [number, number, number] {
...
export function mcuAssignmentDisplayDigitMessage(
  position: number,
  character: string,
  dot = false,
): [number, number, number] {
...
export function xTouchColorMessage(colors: XTouchColor[]): number[] {
  const values: Record<XTouchColor, number> = {
    black: 0,
    red: 1,
    green: 2,
    yellow: 3,
  ...
  return [
    ...
    0x72,
    ...Array.from(
      { length: MCU_CHANNEL_COUNT },
      (_, index) => values[colors[index] ?? "black"],
    ),
    0xf7,
  ];
}

function lcdCharacterCode(character: string): number {
  const code = character.charCodeAt(0);
  return code >= 0x20 && code <= 0x7e ? code : 0x20;
}

export function mcuLcdCellMessage(
  channel: number,
  row: 0 | 1,
  text: string,
): number[] {
...
export function dbToMcuMeterLevel(db: number): number {
  let level = 0;
...
export function mcuMeterMessage(channel: number, db: number): [number, number] {
  const safeChannel = Math.min(MCU_CHANNEL_COUNT - 1, Math.max(0, channel));
  return [0xd0, (safeChannel << 4) | dbToMcuMeterLevel(db)];
}

export function isFullSizeXTouchPort(port: MidiPortDescriptor): boolean {
  const descriptor = `${port.manufacturer ?? ""} ${port.name ?? ""}`;
  return (
    /x[\s_-]?touch/i.test(descriptor) &&
    !/compact|mini|one|extender|\bext\b|midi\s*(?:in|out)?\s*2\b/i.test(
      descriptor,
    )
  );
}
